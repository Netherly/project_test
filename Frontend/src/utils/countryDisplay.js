...
export function normalizeCrmLanguage(language) {
...
export function inferPhoneCountryIso2(phone) {
...
export function buildCountryNames(iso2) {
  const normalizedIso2 = normalizeIso2(iso2);
...
export function buildSyntheticCountryOption(iso2, crmLanguage = "ru") {
  ...
  return {
    ...names,
    id: null,
    value: `__country__:${names.iso2}`,
    label: getCountryDisplayName(names, crmLanguage),
    synthetic: true,
  };
}
