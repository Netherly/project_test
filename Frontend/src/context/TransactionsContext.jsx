...
export const TransactionsProvider = ({ children, authReady, isAuthenticated }) => {
  ...
  const buildPaymentDetailsMap = (item) => {
    const details = String(item?.payment_details ?? item?.paymentDetails ?? '').trim();
  ...
  const [orders, setOrders] = useState(() => sortOrdersList(readCachedValue('ordersData', [])));
  ...
  useEffect(() => {
    ...
    const loadAll = async () => {
      ...
      const resultMap = new Map(requests.map(([key], index) => [key, settled[index]]));
      ...
      if (transactionsRes?.status === 'fulfilled') {
        ...
        setTransactions((prev) => (hasDataChanged(prev, normalized) ? normalized : prev));
