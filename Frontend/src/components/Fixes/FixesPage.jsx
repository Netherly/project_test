...
function Fixes() {
  ...
  const handleSaveFix = useCallback(
    (fixData) => {
      if (selectedFix) {
        setFixes((prev) =>
          prev.map((fix) => (fix.id === selectedFix.id ? { ...fix, ...fixData } : fix))
        );
      } else {
        const newFix = {
      ...
      <div className="fixes-page-main-container">
        ...
        <div className="fixes-table-container">
          <table className="fixes-table">
            <thead>
              <tr>
                <th className="fixes-sortable-header" onClick={() => handleSort("date")}>
                  <span>Дата</span>
                  {renderSortIcon("date")}
                </th>
                <th className="fixes-sortable-header" onClick={() => handleSort("reporter")}>
                  <span>Кто</span>
                  {renderSortIcon("reporter")}
                </th>
                <th>Описание</th>
                <th>Медиа</th>
                <th className="fixes-sortable-header" onClick={() => handleSort("status")}>
                  <span>Статус</span>
                  {renderSortIcon("status")}
                </th>
                <th>Кому</th>
                <th className="fixes-sortable-header" onClick={() => handleSort("type")}>
                  <span>Тип</span>
                  {renderSortIcon("type")}
                </th>
                <th>Исправил</th>
              ...
              {sortedFixes.length === 0 ? (
                ...
                sortedFixes.map((fix) => (
                  ...
                  >
                    ...
                    <td>
                      <span
                        className={`fixes-status fixes-status-${
                          fix.status === "Исправлено" ? "fixed" : "inprogress"
                        }`}
                      >
                        {fix.status}
                      </span>
                    ...
                    <td>
                      <span
                        className={`fixes-type fixes-type-${
                          fix.type === "Фронтенд" ? "frontend" : "backend"
                        }`}
                      >
                        {fix.type}
                      </span>
