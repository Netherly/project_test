...
const ExecutorCard = ({ order, onCardClick, onOpenOrderModal, formatDate }) => {
  ...
  return (
    ...
    >
      ...
      <p className="performer-role">
        Роль: {order.performerRole || 'Роль не указана'}
      </p>

      <div className="card-body">
        <p className="order-name">
          {order.orderName || 'Название заказа отсутствует'}
        </p>

        <p>
          Клиент:{' '}
          <strong>{order.clientHidden ? 'Скрыт' : order.order_main_client}</strong>
        </p>

        <p className="order-description">
          {truncateText(order.orderDescription, 150)}
        </p>

        <p>
          Время работы: <strong>{order.calculatedWorkTime || 'Не указано'}</strong>
        </p>
      </div>
      ...
      <div className="executor-card-footer">
        <div className="footer-amount">
          <span>Сумма:</span>
          <strong>
            {(order.orderSum || 0).toFixed(2)} {order.orderCurrency}
          </strong>
        </div>
        ...
        <div className="footer-amount">
          <span>К оплате:</span>
          <strong>
            {paymentDue.toFixed(2)} {order.orderCurrency}
          </strong>
        </div>
      </div>
    </div>
