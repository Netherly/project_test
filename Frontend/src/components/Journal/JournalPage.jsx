...
const JournalPage = () => {
    ...
    const getActiveOrders = () => {
        ...
        return normalizedOrders.filter(order => activeStatuses.includes(order.status));
    };
    ...
    const applyFilters = () => {
        const {
            searchGlobal,
            searchStatus, searchOrderNumber, searchExecutor, searchRole,
            searchWorkDate, searchWorkDone, searchAdminApproved, searchSource,
        } = filterData;
        ...
        let filtered = allLogEntries.filter(entry => {
            ...
            if (searchGlobal) {
                ...
                const formattedDate = formatDate(entry.workDate).toLowerCase();
            ...
            const matchesOrderNumber = searchOrderNumber.length === 0 || searchOrderNumber.includes(String(entry.orderNumber));
            ...
            return matchesGlobal && matchesStatus && matchesOrderNumber && matchesExecutor && matchesRole &&
    ...
    const handleApplyFilters = () => {
        const mergedFilters = {
            ...tempFilterData,
            searchGlobal: filterData.searchGlobal 
        };
    ...
    const handleQuickFilter = (columnKey, value) => {
        ...
        const newFilters = { ...filterData, [filterKey]: newValues };
        updateURLWithFilters(newFilters);
    };
    ...
    const uniqueStatuses = useMemo(() => {
        const statuses = activeOrders.map(order => order.status).filter(Boolean);
        return [...new Set(statuses)];
    }, [activeOrders]);

    const uniqueOrderNumbers = useMemo(() => {
        const orderNumbers = activeOrders.map(order => String(order.orderNumber)).filter(Boolean);
    ...
    const uniqueWorkDates = useMemo(() => {
        const dates = allLogEntries.map(entry => entry.workDate).filter(Boolean);
        return [...new Set(dates)].sort();
    }, [allLogEntries]);

    const parseHoursToMinutes = (hoursStr) => { if (!hoursStr && hoursStr !== 0) return 0; const s = String(hoursStr).trim(); if (s.includes(':')) { const [h, m] = s.split(':').map(part => parseInt(part, 10) || 0); return h * 60 + m; } const num = parseFloat(s.replace(',', '.')); if (isNaN(num)) return 0; return Math.round(num * 60); };
    const formatMinutesToHMM = (minutes) => { if (!minutes && minutes !== 0) return "00:00"; const m = Number(minutes); const sign = m < 0 ? '-' : ''; const abs = Math.abs(m); const h = Math.floor(abs / 60); const mm = abs % 60; const hh = String(h).padStart(2, '0'); return `${sign}${hh}:${String(mm).padStart(2, '0')}`; };
    const calculatePointsAndPenalty = (workDate, startTime, createdAt = new Date()) => { if (!workDate || !startTime) { return { points: 0, penalty: 0, coefficient: 1 }; } try { const [year, month, day] = workDate.split('-').map(Number); const [hours, minutes] = startTime.split(':').map(Number); const workDateTime = new Date(year, month - 1, day, hours, minutes); const diffMs = createdAt - workDateTime; const diffHours = diffMs / (1000 * 60 * 60); let points = 0; let penalty = 0; let coefficient = 1; if (diffHours <= 24) { points = 1; coefficient = 1; } else if (diffHours <= 48) { points = 0; coefficient = 1; } else if (diffHours <= 72) { penalty = 0.5; coefficient = 0.75; } else { penalty = 1; coefficient = 0.5; } return { points, penalty, coefficient }; } catch (error) { console.error('Ошибка при расчете баллов:', error); return { points: 0, penalty: 0, coefficient: 1 }; } };
    ...
    const calculateDisplayTime = (entry) => {
        ...
        else if (approval === "Корректировка администратором" && entry.correctionTime) { baseTime = entry.correctionTime; } 
    ...
    const adminApprovalOptions = [ "Ожидает", "Принято", "Время трекера", "Время журнала", "Корректировка администратором" ];
