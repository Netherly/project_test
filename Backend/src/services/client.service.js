...
const ClientService = {
  async createClient(payload) {
    ...
    const client = await prisma.client.create({ data, include: includeClient });
    ...
    if (needsRefetch) {
      const updated = await prisma.client.findUnique({ where: { id: client.id }, include: includeClient });
  ...
  async getAllClients() {
    const clients = await prisma.client.findMany({ include: includeClient, orderBy: { name: 'asc' } });
    return clients.map(normalizeClient);
  },

  async getClientById(id) {
    const client = await prisma.client.findUnique({ where: { id }, include: includeClient });
    return normalizeClient(client);
  },

  async updateClient(id, payload) {
    const data = await buildClientData(payload);
    const client = await prisma.client.update({ where: { id }, data, include: includeClient });
    const { hasAccesses, accesses } = extractAccessPayload(payload);
    let needsRefetch = false;
    ...
    if (needsRefetch) {
      const updated = await prisma.client.findUnique({ where: { id }, include: includeClient });
      return normalizeClient(updated);
    }
    return normalizeClient(client);
  },
